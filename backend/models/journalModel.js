...
const getNextVoucherNumber = async (connection) => {
  ...
  const number = parseInt(
    rows[0].voucher_number.replace("JV-", ""),
    10
  );
  ...
  return `JV-${String(nextNumber).padStart(5, "0")}`;
};
...
const createJournalVoucher = async ({
  ...
  try {
    ...
    const voucherNumber =
      await getNextVoucherNumber(connection);

    const [voucherResult] = await connection.execute(
      `INSERT INTO journal_vouchers
       (voucher_number, voucher_date, narration)
       VALUES (?, ?, ?)`,
      [
        voucherNumber,
        voucherDate,
...
const getAllJournals = async () => {
  ...
  rows.forEach((row) => {
    ...
    if (row.entry_id) {
      ...
      journal.total_debit += Number(
        row.debit || 0
      );

      journal.total_credit += Number(
        row.credit || 0
      );
    }
  });

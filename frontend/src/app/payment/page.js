...
export default function PaymentPage() {
  ...
  const [loadingLedgers, setLoadingLedgers] =
    useState(true);
  ...
  const expensePartyLedgers = useMemo(() => {
    return ledgers.filter((ledger) => {
      const name = String(
        ledger.name || ""
      ).toLowerCase();
      ...
      const expenseGroup =
        ...
        group.includes("direct expense") ||
        group.includes("indirect expense") ||
        group.includes("expense");

      const expenseParent =
        parent.includes("purchase") ||
        parent.includes("direct expense") ||
        parent.includes("indirect expense") ||
  ...
  const handleSubmit = async (e) => {
    ...
    if (!form.expenseLedgerId) {
      alert(
        "Please select expense / party ledger"
      );
      return;
    }
    ...
    if (!form.bankLedgerId) {
      alert(
        "Please select bank / cash ledger"
      );
      return;
    }
    ...
    await dispatch(
      createPayment({
        ...
        narration:
          form.narration,
        expenseLedgerId:
          Number(form.expenseLedgerId),
        bankLedgerId:
          Number(form.bankLedgerId),
        amount:
          Number(form.amount),
      })
    );
...
.payment-header { min-height: 96px; display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 20px 22px; margin-bottom: 18px; background: linear-gradient(100deg,#0c1c31,#183d82); border: 1px solid #3c83b7; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,.28); }

.header-content { display: flex; align-items: center; gap: 14px; }

.payment-icon { width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; background: #f59e0b; color: #211403; border-radius: 8px; font-size: 14px; font-weight: 900; box-shadow: 0 5px 18px rgba(245,158,11,.18); }
...
.field label { color: #86a6c0; font-size: 10px; font-weight: 700; text-transform: uppercase; letter-spacing: .4px; }

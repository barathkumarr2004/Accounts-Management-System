...
export default function PurchasePage() {
  ...
  const purchaseLedgers = useMemo(() => {
    return ledgers.filter((ledger) => {
      ...
      return (
        group.includes("purchase account") ||
        group === "purchase" ||
        parent.includes("purchase account") ||
        parent === "purchase" ||
        name === "purchase a/c" ||
        name === "purchase account" ||
        name.includes("purchase a/c")
      );
    });
  }, [ledgers]);
  ...
  const handleSubmit = async (e) => {
    ...
    if (!supplierLedgerId) {
      setMessage(
        "Please select supplier / cash ledger"
      );
      return;
    }
    ...
    const purchaseData = {
      voucherDate,
      supplierLedgerId:
        Number(supplierLedgerId),
      purchaseLedgerId:
        Number(purchaseLedgerId),
      narration,
      items: [
        {
          stockId:
    ...
    if (
      createPurchase.fulfilled.match(result)
    ) {
      setMessage(
...
.purchase-bg { position: fixed; inset: 0; pointer-events: none; background: radial-gradient(700px at 10% 0%, rgba(35,126,184,.12), transparent), radial-gradient(600px at 90% 100%, rgba(245,158,11,.06), transparent); }

.purchase-container { position: relative; max-width: 1180px; width: 100%; margin: 0 auto; }

.purchase-header { min-height: 96px; display: flex; align-items: center; justify-content: space-between; gap: 20px; padding: 20px 22px; margin-bottom: 18px; background: linear-gradient(100deg,#0c1c31,#183d82); border: 1px solid #3c83b7; border-radius: 10px; box-shadow: 0 10px 30px rgba(0,0,0,.28); }

.header-content { display: flex; align-items: center; gap: 14px; }

.purchase-icon { width: 52px; height: 52px; display: flex; align-items: center; justify-content: center; background: #f59e0b; color: #1c1303; border-radius: 8px; font-size: 16px; font-weight: 900; box-shadow: 0 5px 18px rgba(245,158,11,.18); }
